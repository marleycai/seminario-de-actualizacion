...
try {
    if ($username == "" || $password == "") {
        ...
        $SQLStatement = $connection->prepare("CALL `usp_authenticate_user`(:username)");
        ...
        if ($rowCount > 0) {
            ...
            if (password_verify($password, $user['password'])) {
                $status = array("status" => "ok", "description" => "Authentication successful");
            } else {
                $status = array("status" => "error", "description" => "Authentication failed");
            }
        } else {
